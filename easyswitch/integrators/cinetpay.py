...
class CinetpayAdapter(BaseAdapter):
    ...
    def format_transaction(self, data: TransactionDetail) -> Dict[str, Any]:
        ...
        order |= {
            ...
            ),  # ALWAYS MOBILE MONEY SINCE WE ONLY SUPPORT MOBILE MONEY
    ...
    def compare_tokens(self, payload_str: str, recieved_token: str) -> bool:
        ...
        return hmac.compare_digest(token, recieved_token)
    ...
    def validate_webhook(self, payload, headers) -> bool:
        ...
        if not payload:
            raise AuthenticationError(
                message="Invalid payload", provider=self.provider_name()
            )

        # Check if the headers are valid
        if not headers or "x-token" not in headers:
            raise AuthenticationError(
                message="Invalid headers", provider=self.provider_name()
            )
    ...
    def parse_webhook(self, payload, headers):
        ...
        if not self.validate_webhook(payload, headers):
            raise AuthenticationError(
                message="Invalid webhook signature", provider=self.provider_name()
            )
    ...
    async def refund(
        ...
        raise UnsupportedOperationError(
            message="CinetPay does not support refunds", provider=self.provider_name()
        )
